...
; \quad i \in [0,N] \text{ and } N \text{ is a number of assets in portfolio.} 
```
...
Example: Buy 10 ETH @ Limit 1 BTC Fee 0.1 BNB filled at 50%:
...
### Margin Requirement
Assuming we have margin rate $r_j$ assigned to each asset we can calculate margin requirement $R_j$:
...
```math
R_i  = P_i \times \bigg| Q_1 \times f_1 + Q_2 \times f_2 + \dots + Q_N \times f_N \bigg| \times r_j
```
